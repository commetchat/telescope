...
router.get('/url_preview/encrypted/metadata/:userKey/:encryptedUrl/:signature', ({ params, query, url }, env: Env) => {
    return encryptedPreview(params, query as { [key: string]: string }, url, env, );
});

router.get('/url_preview/encrypted/image/:userKey/:encryptedUrl/:signature', ({ params, query, }, env: Env) => {
